...
namespace Product.API.Services
{
    public class ProductService : IProductService
    {
        ...
        //Update số lượng
        public async Task UpdateProductQuantitiesAsync(List<ProductUpdateDTO> productUpdates)
        {
            foreach (var update in productUpdates)
            {
                var product = await _productRepository.GetProductById(update.product_id);
                if (product == null)
                {
                    throw new KeyNotFoundException($"Product with ID {update.product_id} not found.");
                }
